...
live runtime it is a model call (``model_executor``) or a delegated subagent.
The desk itself knows nothing about how a seat thinks, only how to coordinate
it.
...
- ``live`` — the live executor: a Hermes-free model call (``ModelClient``,
  ``build_prompt``, ``parse_finding``, ``model_executor``, ``env_client``)
